...
+ (void) disable
{
	...
	injection->_enabled = NO;

	// Re-swizzling init and dealloc methods
	[NSObject allowInjectionSubscriptionOnInitMethod];

	// Removing file watcher
	injection->_dciDirectoryFileWatcher.delegate = nil;
	injection->_dciDirectoryFileWatcher          = nil;
	NSLog(@"============================================");
...
- (void) newFileWasFoundAtPath:(NSString *)filePath
{
	...
	if ([[filePath lastPathComponent] isEqualToString:@"resource"]) {
		...
		NSString *injectedResourcePath = [NSString stringWithContentsOfFile:filePath
		                                                           encoding:NSUTF8StringEncoding
		                                                              error:nil];
